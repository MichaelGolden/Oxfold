...
public class Benchmarks {
	public static void main(String [] args)
	{
		...
		try {
			Benchmarks.saveBenchmarksCSV(new File("results.csv"), experimentalStructures, experimentalStructures);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public static void saveBenchmarksCSV(File csvFile, List<StructureData> experimentalStructures, List<StructureData> predictedStructures) throws IOException
	{
		...
		for(int i = 0 ; i < experimentalStructures.size() ; i++)
		{
			...
			double mountainSim = MountainMetrics.calculateMountainSimilarity(expStructure.pairedSites, predictedStructure.pairedSites);
