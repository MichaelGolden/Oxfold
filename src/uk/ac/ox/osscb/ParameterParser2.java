...
public class ParameterParser2 {
	...
	public EvolutionaryParameters parse(String pPath) {
		...
		double[][] unpairedR = new double[1][1]; double[][] unpairedD = new double[1][1]; double[][] unpairedRI = new double[1][1]; double[] unpairedprior = new double[1];
		double[][] pairedR = new double[1][1]; double[][] pairedD = new double[1][1]; double[][] pairedRI = new double[1][1]; double[] pairedprior = new double[1];
		String alphstring = "";
		try {
					...
					if (AF*SF == 0) {
						...
						if (tmp.length != length) {
							throw new IllegalArgumentException(String.format(
									"Number of probabilities specified: %d does not match length of alphabet: %d",
									tmp.length, length ));
						}
						for (int j = 0; j<tmp.length; j++) {
