...
public class HelicesMaker {
		 ...
		 * @return said helix
		 */
		public Helix makeHelix(int leftIdx, int rightIdx, BigDecimal[][] diffs, boolean[][] canPair) {
...
/*
 * BigDecimal[][] map = new BigDecimal[length][length];
			for (int j = 0; j<length; j++) {
				map[j][j] = unpairedProbs[j];
				for (int k = j+1; k<length; k++) {
					map[j][k]=BigDecimal.ZERO;
				}
			}
			...
			for (int b = 1; b<rightIdx-leftIdx-1; b++) {
				for (int j = leftIdx + 1; j<rightIdx-b; j++) {
					map[j][j+b] = (unpairedProbs[j].divide(BigDecimal.valueOf(2))).add(map[j+1][j+b],MathContext.DECIMAL32);
			...
			BigDecimal leftup = BigDecimal.ZERO; BigDecimal rightup = BigDecimal.ZERO;
			BigDecimal pair = BigDecimal.ZERO;
			while ((leftIdx-j>=0)&&(rightIdx+j<length)) {
				leftup = (unpairedProbs[leftIdx-j].divide(BigDecimal.valueOf(2))).add(map[j+1][j+b],MathContext.DECIMAL32);
			}
