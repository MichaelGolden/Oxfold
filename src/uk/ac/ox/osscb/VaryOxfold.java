...
public class VaryOxfold {
	public static void main(String [] args)
	{
		...
		File outputDir = new File("output14/");
		...
		for(double delta : deltaList)
		{
			...
			for(int i = 0 ; i < Math.min(end, experimentalStructures.size()) ; i++)
			{
				...
				StructureData predictedStructure = Benchmarks.foldOxfold(outputDir, s.file.getName()+"_oxfold", s.sequences, s.sequenceNames, true,Math.pow(2, delta), false);
